...
class Model(object):
    ...
    def __init__(self,
        ...
        self.saver_all = tf.train.Saver(tf.all_variables(), write_version=saver_pb2.SaverDef.V1)
            ...
            with tqdm(desc='Train: ', total=total) as pbar:
                for epoch in range(self.num_epoch):
                   ...
                   for i,batch in enumerate(self.s_gen.gen(self.batch_size)):
                        ...
                        if num_total % 100 == 0:
                            logging.info('\nstep %f - time: %f, loss: %f, perplexity: %f, precision: %f, batch_len: %f'%(current_step, curr_step_time, step_loss, math.exp(step_loss) if step_loss < 300 else float('inf'), precision, batch_len))
                        loss += step_loss / self.steps_per_checkpoint
